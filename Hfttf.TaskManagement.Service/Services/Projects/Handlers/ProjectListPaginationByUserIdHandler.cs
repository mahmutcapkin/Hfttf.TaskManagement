...
{
   public class ProjectListPaginationByUserIdHandler : BaseProjectHandler, IRequestHandler<ProjectListPaginationByUserIdQuery, PagedResponse<IEnumerable<ProjectResponse>>>
    {
        private readonly IUriService _uriService;
        public ProjectListPaginationByUserIdHandler(IProjectRepository projectRepository, IUriService uriService) : base(projectRepository)
        {
            _uriService = uriService;
        }

        public async Task<PagedResponse<IEnumerable<ProjectResponse>>> Handle(ProjectListPaginationByUserIdQuery request, CancellationToken cancellationToken)
        {
            var validPageNumber = request.PageNumber < 1 ? 1 : request.PageNumber;
            var validPageSize = request.PageSize > 10 ? request.PageSize : 10;
            var pagedData = await _projectRepository.GetListPaginationByUserId(request.UserId,validPageNumber, validPageSize);
