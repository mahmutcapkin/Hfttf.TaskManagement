    ...
    public class EmergencyContactInfoApiManager:IEmergencyContactInfoService
    {
        ...
        public async Task AddAsync(EmergencyContactInfoAdd model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.PostAsync("http://localhost:5000/api/TaskManagementApi/EmergencyContactInfos/", stringContent);
            }
        }
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                await httpClient.DeleteAsync($"http://localhost:5000/api/TaskManagementApi/EmergencyContactInfos/{id}");
        ...
        public async Task UpdateAsync(EmergencyContactInfoUpdate model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var jsonData = JsonConvert.SerializeObject(model);
                var stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                await httpClient.PutAsync("http://localhost:5000/api/TaskManagementApi/EmergencyContactInfos", stringContent);
            }
        }
