    ...
    public class ExperienceApiManager:IExperienceService
    {
        ...
        public async Task AddAsync(ExperienceAdd model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.PostAsync("http://localhost:5000/api/TaskManagementApi/Experiences/", stringContent);
            }
        }
        ...
        public async Task UpdateAsync(ExperienceUpdate model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var jsonData = JsonConvert.SerializeObject(model);
                var stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                await httpClient.PutAsync("http://localhost:5000/api/TaskManagementApi/Experiences", stringContent);
            }
        }
