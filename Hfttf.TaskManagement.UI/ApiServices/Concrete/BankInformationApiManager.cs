    ...
    public class BankInformationApiManager:IBankInformationService
    {
        ...
        public async Task AddAsync(BankInformationAdd model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var responseMessage = await httpClient.PostAsync("http://localhost:5000/api/TaskManagementApi/BankInformations/Insert", stringContent);
            }
        }
        ...
        public async Task UpdateAsync(BankInformationUpdate model)
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                ...
                var jsonData = JsonConvert.SerializeObject(model);
                var stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                var responseMessage = await httpClient.PutAsync("http://localhost:5000/api/TaskManagementApi/BankInformations/Update", stringContent);
            }
        }
