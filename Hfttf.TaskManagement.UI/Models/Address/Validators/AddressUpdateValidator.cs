...
namespace Hfttf.TaskManagement.UI.Models.Address.Validators
{
    public class AddressUpdateValidator : AbstractValidator<AddressUpdate>
    {
        public AddressUpdateValidator()
        {
            ...
            RuleFor(x => x.Country).NotNull().WithMessage(ValidatorMessage.NotNullMessage).WithName("Ülke");
            RuleFor(x => x.Country).MaximumLength(30).WithMessage(ValidatorMessage.LengthWarningMessage);

            RuleFor(x => x.City).NotNull().WithMessage(ValidatorMessage.NotNullMessage).WithName("Şehir");
